...
pub fn init_config() -> Result<()> {
    ...
    crate::log_err!(dirs::verge_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IVerge::template(), Some("# lucky"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::profiles_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IProfiles::template(), Some("# lucky"))?;
        }
        <Result<()>>::Ok(())
    }));
...
/// Return a single-instance guard or activate the existing UI and exit cleanly.
pub fn check_singleton_or_exit() -> single_instance::SingleInstance {
    match check_singleton() {
...
fn activate_existing_window() {
    ...
    let window_title: Vec<u16> = "lucky".encode_utf16().chain(Some(0)).collect();
