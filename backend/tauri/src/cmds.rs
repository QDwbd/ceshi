...
#[tauri::command]
pub async fn change_clash_core(clash_core: Option<lucky::ClashCore>) -> CmdResult {
...
#[tauri::command]
pub async fn get_core_version(core_type: lucky::ClashCore) -> CmdResult<String> {
...
/// pick a local image file to use as the global background
#[tauri::command]
pub async fn pick_background_image() -> CmdResult<Option<String>> {
    ...
    match rx.recv() {
        ...
        Ok(None) => Ok(None),
        Err(_) => Err("failed to pick the background image".into()),
    }
}
...
#[tauri::command]
pub async fn update_core(core_type: lucky::ClashCore) -> CmdResult {
    wrap_err!(
