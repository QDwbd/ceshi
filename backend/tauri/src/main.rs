...
fn main() -> std::io::Result<()> {
    ...
    tauri_plugin_deep_link::prepare("moe.QDwbd.lucky");

    // 单例检测
    let singleton = utils::init::check_singleton_or_exit();
    ...
    // Hold the guard until the end of the program. If another instance is already
    // running (for example from a pinned Windows taskbar icon), the new process
    // activates the existing window and exits without showing an error dialog.
