...
export const TrafficGraph = forwardRef<TrafficRef>((props, ref) => {
  ...
  useEffect(() => {
    ...
    // keep the canvas bitmap size synced with its element size
    const syncSize = () => {
    ...
    // (re)start the drawing loop, recomputing the geometry each time
    const start = () => {
    ...
    // redraw the graph when its container is resized
