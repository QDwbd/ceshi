...
import { pickBackgroundImage } from "@/services/cmds";
...
export const ThemeViewer = forwardRef<DialogRef>((props, ref) => {
  ...
  // pick a local image as the global background
  const onPickImage = useLockFn(async () => {
    try {
      const path = await pickBackgroundImage();
      if (!path) return;
      setTheme((t) => ({ ...t, background_image: path }));
    } catch (err: any) {
      useNotification({
        title: t("Error"),
        body: err.message || err.toString(),
  ...
  const onClearImage = () => {
    setTheme((t) => {
      const { background_image: _, ...rest } = t;
      return rest;
    });
  };
      ...
      <List sx={{ pt: 0 }}>
        ...
        <Item>
          ...
          <Select
            ...
            onChange={handleChange("theme_mode")}
            sx={{ width: 135 }}
          >
        ...
        <Item>
          <ListItemText primary={t("Background Image")} />
          <Box display="flex" alignItems="center" gap={1}>
            {theme.background_image ? (
              <Box
                component="img"
                src={convertFileSrc(theme.background_image)}
                alt="background"
                onError={(e) => {
                  // avoid showing a broken image icon when the file is missing
            ...
            <Button size="small" variant="outlined" onClick={onPickImage}>
              {t("Choose File")}
            </Button>
            ...
            <Button
              size="small"
              color="error"
              disabled={!theme.background_image}
